...
$(document).ready(function()
{
	...
	*/
			...
			qtys.each(function()
			{
				...
				sub.on('click', function()
				{
					...
					if(original > 0)
						{
							newValue = original - 1;
							var order = qty.find('.order_id').val();
							var count_price = newValue * qty.find('.price').val();
					        $('.product_total_'+order).html('$'+count_price);
					        $('.product_total_'+order).attr('attr-price', count_price);
							subTotal();
						}
					num.text(newValue);
				});
	...
	function subTotal() {
		var total = 0;
		$('.subtotalprice').each(function(index, el) {
			total += parseInt($(this).attr('attr-price'));
		});
		$('.extra_subtotal').html('$'+total);
		$('.extra_total').html('$'+total);
	}
